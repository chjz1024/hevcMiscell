...
{
	qp = std::min(51,std::max(qp,0));

	int slope = (initValue >> 4) * 5 - 45;
	int offset = ((initValue & 15) << 3) - 16;
	int initState = std::min(std::max(1, (((slope * qp) >> 4) + offset)), 126);
	uint32_t mpState = (initState >= 64);
	ucState = ((mpState ? (initState - 64) : (63 - initState)) << 1) + mpState;
}

const uint8_t ContextModel::TransMPSTable[64<<1] =
{
...
const uint8_t ContextModel::TransLPSTable[64<<1] =
{
...
const int ContextModel::entropyBits[64 << 1] =
{
